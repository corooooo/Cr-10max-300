...
void GcodeSuite::M408() {
  ...
  SERIAL_ECHO(card.isStillPrinting() ? F("\"P\"") : F("\"I\""));
  ...
  // pos: tool position
  tmp[0] = motion.position.x; tmp[1] = motion.position.y; tmp[2] = motion.position.z;
  json_array_print(PSTR("pos"), tmp, 3);

  // extr: extruder position
  for (uint8_t e = 0; e < EXTRUDERS; e++) tmp[e] = motion.position.e;
  ...
  LOOP_NUM_AXES(i) {
    SERIAL_CHAR(i ? ',' : '[');
    SERIAL_ECHO(int(motion.axis_was_homed(AxisEnum(i))));
  }
  SERIAL_CHAR(']');

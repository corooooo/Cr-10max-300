...
void Marlin::idle(const bool no_stepper_sleep);
...
struct TDelay {
  ...
  FI void idle(const bool nss=false) const { while (pending()) marlin.idle(nss); }
  FI void wait_while(testFunc_t fn) const { while (pending() && fn()) { /* wait */ }; }
  FI void wait_until(testFunc_t fn) const { while (pending() && !fn()) { /* wait */ }; }
  FI void idle_while(testFunc_t fn, const bool nss=false) const { while (pending() && fn()) marlin.idle(nss); }

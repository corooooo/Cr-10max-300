...
void Motion::prepare_line_to_destination() {
  ...
  void Motion::homeaxis(const AxisEnum axis) {
    ...
    if (bump) {
      ...
      //if (axis == Z_AXIS) position.z -= probe.offset.z; //rock_20220801 手动对高功能。
...
void Motion::set_axis_is_at_home(const AxisEnum axis) {
   ...
   */
    ...
    if (axis == Z_AXIS) {
        ...
        SERIAL_ECHOLNPGM(" position.z_old=: ", position.z);
        position.z -= probe.offset.z;
        SERIAL_ECHOLNPGM(" probe.offset.z=: ", probe.offset.z);
        SERIAL_ECHOLNPGM(" position.z_new=: ", position.z);
     ...
     if (axis == Z_AXIS) position.z -= probe.offset.z;
  #endif
  #endif
    ...
    if (axis == Z_AXIS) {
        ...
        if (st_bedNozzleHeightCal.bedNozzleHeightCalFinishFlag)
          position.z -= probe.offset.z + st_bedNozzleHeightCal.zCoordinateOffset;
      #else
        //position.z -= probe.offset.z;
      #endif
    }
  ...
  if (DEBUGGING(LEVELING)) {
    ...
    float Motion::homeaxis_bl(const AxisEnum axis, const feedRate_t fr_mm_fast, const feedRate_t fr_mm_slow) {
      ...
      if (ENABLED(USER_LEVEL_DEBUG)) {
        SERIAL_ECHOLNPGM("\nhomeaxis_bl start position.z * 100 = ", position.z * 100);
      ...
      position[axis] = 0 - probe.offset.z;

      if (ENABLED(USER_LEVEL_DEBUG))
        SERIAL_ECHOLNPGM("\nhomeaxis_bl probe.offset.z * 100 = ", probe.offset.z * 100,
                          "\nhomeaxis_bl position[Z] * 100 = ", position[axis] * 100);
      ...
      if (bump) {
        ...
        if (ENABLED(USER_LEVEL_DEBUG))
          SERIAL_ECHOLNPGM("\nhomeaxis_bl probe.offset.z_2 * 100 = ", probe.offset.z * 100,
                            "\nhomeaxis_bl position[Z]_2 * 100 = ", position[axis] * 100);
      ...
      if (DEBUGGING(LEVELING)) {
        SERIAL_ECHOLNPGM("\nhomeaxis_bl end position.z * 100 = ", position.z * 100);
        SERIAL_EOL();
      }
    ...
    void Motion::homeaxis_bl(const AxisEnum axis) {
      ...
      // Put away the Z probe
        ...
        if (endstop_backoff[axis]) {
          position[axis] -= ABS(endstop_backoff[axis]) * axis_home_dir;
          goto_current_position(TERN_(HOMING_Z_WITH_PROBE, axis == Z_AXIS ? MMM_TO_MMS(Z_PROBE_FEEDRATE_FAST) :) homing_feedrate(axis));

...
inline void RTS_line_to_current(AxisEnum axis) {
  if (!planner.is_full())
    planner.buffer_line(motion.position, MMM_TO_MMS(manual_feedrate_mm_m[(int8_t)axis]), active_extruder);
}
...
void RTS::sdCardInit() {
  if (sdDetected()) // If a card is detected
    card.mount();

  DEBUG_ECHOLNPGM(" card.flag.mounted=: ", card.flag.mounted);

  if (card.flag.mounted) { // Mounted successfully
    fileCnt = card.get_num_items();
    card.getWorkDirName();
    if (card.filename[0] != '/') card.cdup();
    hal.watchdog_refresh(); // Troubleshooting the problem of inserting and removing cards causing a reboot - Rock_20220915
  ...
  switch (Checkkey) {
    case MainEnterKey:
      ...
      if (recdat.data[0] == 1) {
        ...
        hmiFlag.leveling_flag = true; // Enter the leveling page
        ...
        CardRecbuf.recordcount = -1; // Unselected
    ...
    case StopPrintKey:
      if (recdat.data[0] == 1) { // Stop printing pop-up window entry
        ...
        if (!PoweroffContinue) { // rock_20220708 Cloud printing
          SERIAL_ECHOLNPGM("ok");
          while ((planner.movesplanned() < 2) && (motion.destination != motion.position)) marlin.idle();
          queue.clear();
          delay(20);
        ...
        if (hmiFlag.G29_flag || hmiFlag.home_flag) break;
        ...
        if (card.isPrinting() || marlin.wait_for_heatup || hmiFlag.home_flag) { // 暂停
          sendData(ExchangePageBase + 10, ExchangepageAddr);
          change_page_font = 10;
        }
      ...
      else if (recdat.data[0] == 5) { // Pull out the card
        if (PoweroffContinue) {
          sendData(ExchangePageBase + 40, ExchangepageAddr);
          change_page_font = 40;
          waitway = 7;
    ...
    case PausePrintKey:
      if (recdat.data[0] == 1) {
        ...
        // Troubleshooting the issue of repeatedly clicking pause before starting a print causing it to restart - rock_20220714
        if (hmiFlag.G29_flag || hmiFlag.home_flag || marlin.wait_for_heatup) break;
        sendData(ExchangePageBase + 11, ExchangepageAddr);
        change_page_font = 11;
      }
      else if (recdat.data[0] == 2) {
    ...
    case ResumePrintKey:
      if (recdat.data[0] == 1) {
        ...
        //if (card.flag.mounted)
        if (card.isSDCardInserted()) { // Has SD Card
          SD_Card_status = true;
          card.startOrResumeFilePrinting();
          print_job_timer.start();
          update_time_value = 0;
          sdcard_pause_check = true;
          sd_printing_autopause = false;
          hmiFlag.remove_card_flag = false;
          ...
          sdCardUpate();
          //card.mount();
          //SERIAL_ECHO_MSG("ROCK_MOVE_CARD!!!!\n");
    ...
    case PrepareEnterKey:
      if (recdat.data[0] == 1) {
        ...
        #endif
        hmiFlag.flag_counter_wifi_reset = true;
        ...
        hmiFlag.leveling_flag = false;

        gcode.process_subcommands_now(F("M420 S1")); // 退出调平界面
        if (!planner.has_blocks_queued()) {
          sendData(10 * motion.position.x, AXIS_X_COORD_VP);
          sendData(10 * motion.position.y, AXIS_Y_COORD_VP);
          sendData(10 * motion.position.z, AXIS_Z_COORD_VP);
          sendData(ExchangePageBase + 21, ExchangepageAddr);
          change_page_font = 21;
        }
        settings.save();
    ...
    case AutoHomeKey:
      if (recdat.data[0] == 1) {
        ...
        waitway = 4;
        if ((motion.axis_is_trusted(X_AXIS)) && (motion.axis_is_trusted(Y_AXIS)) && (motion.position.x >= 50) && (motion.position.y >= 50)) {
    ...
    case XaxismoveKey:
      if (!planner.has_blocks_queued()) {
        ...
        motion.position.x = 0.1f * recdat.data[0];

        LIMIT(motion.position.x, x_min, x_max);
    ...
    case YaxismoveKey:
      if (!planner.has_blocks_queued()) {
        ...
        motion.position.y = constrain(0.1f * recdat.data[0], y_min, y_max);
    ...
    case ZaxismoveKey:
      if (!planner.has_blocks_queued()) {
        ...
        motion.position.z = constrain(0.1f * recdat.data[0], z_min, z_max);
    ...
    case HeaterLoadEnterKey:
      if (!planner.has_blocks_queued()) {
        ...
        #endif
        motion.position.e += FilamentLOAD;
    ...
    case HeaterUnLoadEnterKey:
      if (!planner.has_blocks_queued()) {
        ...
        #endif
        motion.position.e -= FilamentUnLOAD;
    ...
    case StartFileKey:
      if ((recdat.data[0] == 1) && sdDetected()) {
        ...
        if (card.flag.mounted) {
          ...
          sendData(zprobe_zoffset * 100, AUTO_BED_LEVEL_ZOFFSET_VP);
          sendData(ExchangePageBase + 10, ExchangepageAddr);
          queue.enqueue_now(F("G28XY")); // The selected file needs to be printed and heated on a platform that supports the process - rock_20220707
          update_time_value = 0;
          // 1:cloud print start
          SERIAL_ECHOLNPGM("M79 S1");
        }
...
void RTS::eachMomentUpdate() {
  ...
  if (hmiFlag.flag_counter_wifi_reset) {
    ...
    if (wifiresetcount_ms > TIME_WIFI_RESET_BACKPAGE) {
      hmiFlag.flag_counter_wifi_reset = false;
  ...
  if (ms > next_rts_update_ms) {
    ...
    if (card.isPrinting() && last_cardpercentValue != pct) {
      ...
      sendData(10.0f * motion.position.z, AXIS_Z_COORD_VP);
    }
...
void RTS_Update() {
  ...
  if (!hmiFlag.home_flag && !hmiFlag.G29_flag) // Cards are not detected during homing and leveling
    rts.sdCardUpate();
  ...
  if (!marlin.wait_for_heatup && !hmiFlag.home_flag && !hmiFlag.G29_flag) Remove_card_window_check(); // rock_20220709
  hal.watchdog_refresh();
...
bool RTS::levelStepHeightMeasure() {
    ...
    // bltouch+lower limit enabled
      ...
      //motion.destination.set(test_offset_xy, motion.position.z);
      ////motion.destination.set(safe_homing_xy, motion.position.z);
      //TERN_(1, motion.destination -= probe.offset_xy);
      //if (position_is_reachable(motion.destination))
      //  motion.blocking_move_xy(motion.destination);  // Move bltouch to the middle
      //else
      ...
      //coordinateOffset = motion.homeaxis_bl(Z_AXIS, Z_MEASURE_FEEDRATE_FAST, Z_MEASURE_FEEDRATE_SLOW);
  ...
  switch (st_bedNozzleHeightCal.bedNozzleHeightState) {
    ...
    case 1:
      ...
      if (st_bedNozzleHeightCal.goHomeSta == GO_HOME_DONE) {
        // set homing status - 'GO_HOME_IDLE'
        st_bedNozzleHeightCal.goHomeSta = GO_HOME_IDLE;
        DEBUG_ECHOLNPGM("\n----------------------motion.position.z!!!!!!= ", motion.position.z * 100);

        // The coordinates at this point should be Z_MAX_POS
        DEBUG_ECHOLN(motion.position.z * 100);
        motion.destination.set(safe_homing_xy, motion.position.z);

        motion.destination -= probe.offset_xy; // TODO:OPTIONAL?
...
void LcdAutoUIMoveXYBlock(float _posX, float _posY) {
  motion.blocking_move_xy(_MIN(_posX, X_BED_SIZE), _MIN(_posY, Y_BED_SIZE));
}
...
void LcdAutoUIMoveZBlock(float _posZ) {
  motion.blocking_move_z(_MIN(_posZ, Z_MAX_POS));
}
...
void RTS_UpdatePosition() {
  rts.sendData(10 * motion.position.x, AXIS_X_COORD_VP);
  rts.sendData(10 * motion.position.y, AXIS_Y_COORD_VP);
  rts.sendData(10 * motion.position.z, AXIS_Z_COORD_VP);
}
